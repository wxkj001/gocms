    ...
    <MeModal ref="modalRef" width="100%" :modal-style="{ 'height': '100vh', 'align-self': 'auto' }">
      <n-scrollbar style="max-height: 80vh;">
        <n-form ref="modalFormRef" label-placement="left" label-align="left" :label-width="80" :model="modalForm">
          <n-form-item
            v-for="(item, key) in formFields" :key="key" :label="item.name" :path="item.code"
            :rule="ruleFun(item)"
          >
            <AiEditor v-if="item.field_type === 'richtext'" v-model:model-value="modalForm[item.code]" />
            ...
            <n-select
              v-if="item.field_type === 'enum'" v-model:value="modalForm[item.code]"
              :options="item.enum_options"
            />
            <n-date-picker
              v-if="item.field_type === 'datetime'" v-model:formatted-value="modalForm[item.code]" value-format="yyyy-MM-dd HH:mm:ss"
              type="datetime" clearable
            />
            <n-date-picker
              v-if="item.field_type === 'date'" v-model:value="modalForm[item.code]" type="date"
              clearable
            />
            <n-upload
              v-if="item.field_type === 'file'" multiple directory-dnd
